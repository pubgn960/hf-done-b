...
class TestIgnoreAdminAndDeliveryUserMessages(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_admin_and_delivery_user_detection(self):
        ...
        # Super Admin checks
        for sa_id in (1573531032, 6664448946, 8411246465):
            self.assertTrue(is_super_admin(sa_id))
            self.assertTrue(is_delivery_user(sa_id))
...
class TestTwoGroupDatabaseWorkflow(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_two_group_workflow(self):
        ...
        email = "twogroup_workflow_unique@example.com"
        ...
        await set_order_loader_message_id(order.id, 990101)
        loader_order = await get_order_by_loader_msg_id(990101)
