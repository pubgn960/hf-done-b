...
async def source_group_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Group B Workflow: Forward to Payment Review Group (-1004441603990) with Accept / Reject buttons
    ...
    if payment_group_id:
        ...
        logger.warning(f"[PAYMENT] Order #{order.id} registered, but Payment Review Group is not configured yet!")
...
async def users_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    admins = user_groups.get("admin", [1573531032, 6664448946, 8411246465])
